...
class NotificationTest extends DuskTestCase
{
     ...
     * @return void
     */
    public function user_notification_test()
    {
        ...
        $this->browse(function (Browser $browser) use($user){
            ...
            $browser->cookie('browser_id', $verify->browser_id)
                ...
                ->click('#notify-dusk')
                ->assertsee('All')
                ->select('select-type', 'unread=1')
                ->assertsee('Unread')
                ->select('select-type', 'read=1')
                ->assertsee('Your Order now')
                ->clickLink('All')
                ->assertsee('Your Order now')
                ->clickLink('View')
                ->assertsee('Order Placed By')
                ->clickLink('2011_1254828')
                ->waitForText('Ordered on')
