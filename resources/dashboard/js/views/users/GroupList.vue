                ...
                <div class="card">
                    <div class="card-header">
                        ...
                        <div class="float-right">
                            <router-link to="/dashboard/users/groups/create" class="btn btn-primary" tag="button" type="button" style="margin-top: 0px;">
                    ...
                    <div class="card-body">
                            ...
                            <div class="col-lg-2">
                                <div class="row">
                                    ...
                                    <div class="col-md-6 per-page">
                                        <b-form-select v-model="perlist" @input="groupList">
                        ...
                        <table class="table">
                            ...
                            <tr v-for="group,key in groups">
                                <td><input type="checkbox" v-model="Ids" @change="select" :value="group.id"></td>
                                <td>{{group.title}}</td>
                                <td>

                                    <router-link :to="{name: 'Edit Group', params: {id: group.id}}"
                                                 class="btn btn-success btn-sm" tag="button" type="button">
                                        <i class="fa fa-edit"></i>
                                    </router-link>

                                    <b-button v-if="Ids.length==''" variant="warning" size="sm" @click="deleteGroup(group.id, key)"> <i class="fa fa-trash"></i></b-button>
                        ...
                        <pagination :pagination="pagination" @paginate="groupList"
...
<script>
    ...
    export default {
        ...
        methods:{
            ...
            groupList(st = false){
                axios.get('/dashboard/group?page=' + this.pagination.current_page + '&field=' + this.field + '&q=' + this.query + '&per_page=' + this.perlist)
                    .then(response => {
            ...
            deleteGroup(id, key){
                if (!confirm('Are you really want to delete'))
                    return;
                axios.delete('/dashboard/group/'+id ).then((response) => {
                    this.groupList()
                    this.Ids = [];
            ...
            selectAll() {
                ...
                this.Ids = [];
                if (this.allSelected) {
                    for (this.group in this.groups) {
                        this.Ids.push(this.groups[this.group].id);
                    }
                }
            },
            select() {
