...
class ProductDetailController extends Controller
{
    ...
    public function getDetail(Request $request)
    {
        foreach ($request->items as $req) {
            ...
            if ($this->checkProhibited( $originalData )) {
                continue;
            }
    ...
    public function parseAsin($url)
    {
        ...
        // get the html
        $client = new Client();
        $res = $client->get( $data['url'], ['proxy' => config( 'configuration.proxy_id_pass' )] );
        ...
        $data ['title'] = $selectorElements ? $selectorElements [0] ['text'] : '';
        ...
        if(empty($data['current_price']) && $price=select_elements("#priceblock_saleprice",$html))
            $data['current_price']=$price[0]?$price[0]['text']:'';
        ...
        // ==================== Product Category ====================
        if (stripos( $html, '<div id="wayfinding-breadcrumbs_container' ) > -1) {
            $rest = substr(
                $html,
                stripos( $html, '<div id="wayfinding-breadcrumbs_container' ),
                strlen( $html )
            );
...
// now use the dom parser
    ...
    private function checkProhibited($item)
    {
        $status = false;
        if (isset( $item['category'] )) {
            $categories = array_map( 'trim', explode( '>', $item['category'] ) );
            array_pop( $categories );
            if (ProhibitedItem::whereType( 'category' )->whereIn( 'title', $categories )->exists()) {
                $prohibitedItem = ProhibitedItem::whereType( 'title' )->where( function ($query) use ($item) {
                ...
                if ($prohibitedItem->exists()) {
                    ...
                    foreach ($items as $data) {
                        if (stripos( $item['title'], $data->title ) !== false) {
                            $this->storeProhibitedItem( $item );
                            $status = true;
                            break;
                        }
                    }
    ...
    private function storeProhibitedItem($data)
    {
        $item['img'] = $data['imageUrl'];
        $item['url'] = $data['url'];
        $item['title'] = $data['title'];
        Redis::sadd( request( 'sessionId' ), json_encode( $item ) );
    }
}
